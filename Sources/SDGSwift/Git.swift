...
public enum Git {
  ...
  public static let compatibleVersionRange: Range<Version> = Version(
    1,
    9,
    0
  )..<Version(2).compatibleVersions.upperBound
  ...
  private static func tool() -> Result<ExternalProcess, LocationError> {
    return cached(in: &located) {
      ...
      let searchLocations = Git.searchCommands.lazy.compactMap({
