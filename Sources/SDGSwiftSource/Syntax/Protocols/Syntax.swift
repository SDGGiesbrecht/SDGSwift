...
extension Syntax {
    ...
    public func api() -> [APIElementKind] {
        ...
        case let structure as StructDeclSyntax :
            return structure.typeAPI.flatMap({ [APIElementKind(element: $0)] }) ?? []
        case let `class` as ClassDeclSyntax :
            return `class`.typeAPI.flatMap({ [APIElementKind(element: $0)] }) ?? []
        case let enumeration as EnumDeclSyntax :
            return enumeration.typeAPI.flatMap({ [APIElementKind(element: $0)] }) ?? []
        case let typeAlias as TypealiasDeclSyntax :
            return typeAlias.typeAPI.flatMap({ [APIElementKind(element: $0)] }) ?? []
        case let associatedType as AssociatedtypeDeclSyntax :
            return associatedType.typeAPI.flatMap({ [APIElementKind(element: $0)] }) ?? []
        case let `protocol` as ProtocolDeclSyntax :
            return `protocol`.protocolAPI.flatMap({ [APIElementKind(element: $0)] }) ?? []
        case let `case` as EnumCaseDeclSyntax :
            return `case`.caseAPI().map({ APIElementKind(element: $0) })
        case let initializer as InitializerDeclSyntax :
            return initializer.initializerAPI.flatMap({ [APIElementKind(element: $0)] }) ?? []
        case let variable as VariableDeclSyntax :
            return variable.variableAPI().map({ APIElementKind(element: $0) })
        case let `subscript` as SubscriptDeclSyntax :
            return `subscript`.subscriptAPI.flatMap({ [APIElementKind(element: $0)] }) ?? []
        case let function as FunctionDeclSyntax :
            return function.functionAPI().flatMap({ [APIElementKind(element: $0)] }) ?? []
        case let `extension` as ExtensionDeclSyntax :
            return `extension`.extensionAPI.flatMap({ [APIElementKind(element: $0)] }) ?? []
