...
public class DocumentationParameterList : DocumentationCallout {
    ...
    internal init(callout: DocumentationCallout, parameters: [DocumentationListElement], in source: String) {
        ...
        for parameter in parameters {
            if let parsed = DocumentationParameterListEntry(entry: parameter, in: source) { // @exempt(from: tests) False result in Xcode 9.3.
                parameterList.append(parsed)
            }
        }
        self.parameters = parameterList
        super.init(bullet: callout.bullet, callout: callout.callout, colon: callout.colon, end: parameterList.last?.range.upperBound ?? callout.range.upperBound, in: source, knownChildren: parameterList) // @exempt(from: tests) After ?? would require invalid syntax.
    }
