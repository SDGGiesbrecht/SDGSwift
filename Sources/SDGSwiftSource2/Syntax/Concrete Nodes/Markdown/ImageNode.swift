...
public struct ImageNode: StreamedViaChildren, SyntaxNode {
  ...
  internal init?(components: [SyntaxNode]) {
    ...
    self.delimiter = Token(kind: .imageDelimiter(String(leadingDelimitersRemainder.removeFirst())))

    guard
      let link = LinkNode(
        components: [Token(kind: .linkDelimiter(String(leadingDelimitersRemainder)))].appending(
