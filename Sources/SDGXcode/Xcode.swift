...
public enum Xcode {
  ...
  internal static let compatibleVersionRange = Version(11, 2, 0) /* Travis CI */
  ...Version(11, 2, 1) /* Current */
  ...
  private static func tool() -> Result<ExternalProcess, LocationError> {
    return cached(in: &located) {
      ...
      let searchLocations = Xcode.searchCommands.lazy.compactMap({
        SwiftCompiler._search(command: $0)
      })
  ...
  @discardableResult public static func build(
    ...
    case .success(let scheme):  // @exempt(from: tests) Unreachable on Linux.
      ...
      return runCustomSubcommand(command, in: package.location, reportProgress: reportProgress)
