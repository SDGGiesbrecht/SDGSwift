...
public enum Xcode {
    ...
    internal static let versions = Version(10, 0, 0) /* Travis CI */ ... Version(10, 0, 0) /* Current */
    ...
    public static func warningsOccurred(during log: String) -> Bool {
        let warnings = log.scalars.matches(for: " warning:".scalars).filter { match in // @exempt(from: tests) Only triggered on the first build.
            return ¬log.scalars[match.range.upperBound...].hasPrefix(" directory not found for option \u{27}\u{2D}F/Applications/Xcode".scalars) // @exempt(from: tests)
