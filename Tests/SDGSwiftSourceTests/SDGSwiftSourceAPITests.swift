...
class SDGSwiftSourceAPITests : TestCase {
    ...
    func testFragment() {
        ...
        let fragment = FragmentSyntax(scalarOffsets: 5 ..< 10, in: scanned)
        XCTAssertEqual(fragment.text, "spans")
        let croppedFragment = FragmentSyntax(scalarOffsets: 2 ..< 7, in: scanned)
    ...
    func testParsingSwiftSyntax() {
        do {
            for url in try FileManager.default.deepFileEnumeration(in: beforeDirectory) {
                ...
                let api = APIScanner().visit(sourceFile).source()
