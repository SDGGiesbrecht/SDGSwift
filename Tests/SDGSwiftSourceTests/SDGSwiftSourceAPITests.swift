...
class SDGSwiftSourceAPITests : TestCase {
    ...
    func testTemporary() {
        ...
        do {
            for url in try FileManager.default.deepFileEnumeration(in: URL(fileURLWithPath: #file).deletingLastPathComponent().deletingLastPathComponent().deletingLastPathComponent().deletingLastPathComponent().appendingPathComponent("SDGCornerstone")) where url.lastPathComponent == "DataStream.swift" {//url.pathExtension == "swift" {
                print(url.path)
